...
test("parseTrackForLyrics - handles square brackets and tags", () => {
  const track = {
...
test("parseTrackForLyrics - cleans Topic and VEVO channel suffixes", () => {
  const trackTopic = {
...
test("parseTrackForLyrics - handles feat in artist segment", () => {
  const track = {
...
test("parseTrackForLyrics - handles null or empty track gracefully", () => {
...
test("handleLyrics - returns undefined when no track exists", async () => {
  const res = await handleLyrics({});
  assert.equal(res, undefined);
});

test("isTestEnvironment - returns true in test environment", () => {
